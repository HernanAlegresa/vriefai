...
export default function GenerationDetailPage({
  ...
  const { getGeneration, loading: generationsLoading } =
  ...
  return (
    <div className="min-h-full px-4 py-8 md:px-8 md:py-10">
      <div className="mx-auto max-w-5xl">
        ...
        {/* Header card — overflow-hidden for the color stripe */}
        <motion.div
        ...
        >
          <div className="relative px-5 pb-5 pt-6 md:px-6 md:pb-6">
            ...
            {generation.briefMensual && (
              <div className="mt-4 rounded-xl border border-[#ebe5dc] bg-[#faf8f4] p-4">
                <p className="mb-1.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#8b8498]">
                  Brief del mes
                </p>
                <p className="whitespace-pre-wrap text-sm leading-6 text-[#625d6d]">
                  {generation.briefMensual}
                </p>
              </div>
            )}
        ...
        {/* Content viewer — no overflow-hidden so sticky tabs work */}
        <motion.div
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-xl border border-[#ebe5dc] bg-[#faf8f4] px-4 py-3">
      ...
      <p className="mt-0.5 font-display text-2xl font-bold text-[#171422]">
        {value}
      </p>
    </div>
  );
